...
> **当前版本**: v0.3.0-dev (Learning层开发中) | v0.2.0 - 轻量级语义搜索版本

基于 FastAPI + DeepSeek + PostgreSQL 的多轮对话智能助手,支持上下文记忆、会话管理、数据持久化、用户行为分析。
...
**当前状态**：v0.2.0 已完成存储层基础功能，v0.3.0 正在开发学习层（行为分析、冲突检测已完成）。

## ✨ 核心特性

- 🧠 **智能记忆系统**：PostgreSQL持久化，支持关键词搜索、时间筛选
- 🔍 **语义搜索**：基于TF-IDF的轻量级语义理解，无需深度学习
- 💬 **多轮对话**：完整的上下文管理，支持会话历史回溯
- � **用户行为分析**：活跃时间分布、话题偏好、对话统计（v0.3.0）
- ⚠️ **记忆冲突检测**：自动识别矛盾的关键信息（v0.3.0）
- �🔄 **自动重试**：API调用失败自动重试，指数退避策略
- 📝 **详细日志**：完整的请求/响应日志，便于调试和监控
- 🌐 **Web界面**：简洁美观的聊天界面，支持会话管理、记忆查看和行为分析
...
### 🚧 v0.3.0 - Learning层初始版本（开发中）

基于架构愿景的**Learning（学习层）**功能：

#### 1. 行为分析（Behavior Analysis）
- [x] 用户对话模式分析（活跃时间、话题偏好）
- [x] 记忆冲突检测（姓名、年龄、生日等矛盾识别）
- [x] 用户行为统计面板（前端可视化）
- [ ] 记忆标签自动分类优化
...
**架构对照**：v0.2.0专注于**Storage层**，v0.3.0正在建设**Learning层**（行为分析已完成），v0.4.0+将实现**Active Perception层**。
