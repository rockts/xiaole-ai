...
### ✅ 已完成（2025-11-15）
- 顶部导航透明化并居中展示会话标题，统一桌面/移动端交互
- 侧边栏与聊天区重新布局，恢复“左侧导航、右侧会话”结构并适配折叠逻辑
- 聊天气泡与头像重新设计，统一用户/助手样式与动效，补全删除操作兼容性
- `newChat()` 与会话加载流程修正，确保新建对话自动切回聊天页并同步 URL 状态
- 响应式样式更新：移动端滚动、间距、输入栏与按钮尺寸适配
- 创建 `static/css/app.css`，将布局与主题变量从 `index.html` 抽离为独立样式文件

### 🗺️ 规划中
- 拆分 `static/index.html` 为模块化结构（布局壳、侧边栏、聊天窗、设置等组件）
- 抽离样式与脚本至 `static/css`、`static/js`，建立统一打包/加载策略
- 统一对话框 UI（欢迎页、新建对话、聊天窗口）并复用组件化模板
- 为组件化重构补充文档、迁移指南与回归测试清单
