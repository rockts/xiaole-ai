...
def optimize_memory():
    ...
    try:
        ...
        print(f"   ✅ 清理了 {len(duplicate_facts)-1 if len(duplicate_facts)>1 else 0} 条重复facts")
        ...
        conversations_count = db.query(Memory).filter(Memory.tag.like('conversation:%')).count()
